...
def get_ret_ifo(xy_list: list, slide, svs_address,
    ...
    tiles_dir, _ = os.path.splitext(filename)
    if not os.path.exists(os.path.join(image_dir_split, tiles_dir)):
        os.mkdir(os.path.join(image_dir_split, tiles_dir))
    image_address = image_dir_split + '/' + tiles_dir + '/' + svs_address.split('/')[-1]
